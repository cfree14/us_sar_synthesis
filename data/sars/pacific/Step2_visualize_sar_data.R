...
# Subset
sdata <- data %>% 
  filter(stock=="Humpback whale (CA/OR/WA)")
  # filter(stock=="California sea lion (USA)")

# Prep N data
sdata1 <- sdata %>% 
  select(year, n_est, n_min) %>% 
  gather(key="metric", value="n", 2:ncol(.)) %>% 
...
g1 <- ggplot(sdata1, aes(x=year, y=n, color=metric, group=metric)) +
  geom_line() +
  geom_point() + 
  # Labels
  labs(x="Year", y="Number of animals",  tag="A") +
  # Legend
...
g2 <- ggplot(sdata, aes(x=year, y=rf)) +
  ...
  # Labels
  labs(x="Year", y="Recovery factor (RF)", tag="B") +
  # Theme
  theme_bw() + my_theme
g2
